...
ワークフローは`.github/workflows/claude-review.yml`にあります。GitHub Appを使わず、同一リポジトリに限定した`GITHUB_TOKEN`をClaude Actionへ渡します。レビューにはPRへのコメント権限、リポジトリ内容の読み取り権限、Actions実行結果の読み取り権限を付与しています。Claude Action v1の実行時認証でOIDCトークン取得が必要になるため、`id-token: write`も付与しています。APIキーをOIDC認証へ変更する設定ではありません。
...
同じPRへ短時間に複数のcommitをpushした場合、古いレビュー実行はconcurrency制御でキャンセルされます。Claudeのレビュー処理には15分のタイムアウトがあります。
