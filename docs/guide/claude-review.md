...
ワークフローは`.github/workflows/claude-review.yml`にあります。GitHub Appを使わず、同一リポジトリに限定した`GITHUB_TOKEN`をClaude Actionへ渡します。レビューにはPRへのコメント権限、リポジトリ内容の読み取り権限、Actions実行結果の読み取り権限、Claude Actionの認証に必要なOIDCトークン発行権限を付与しています。
